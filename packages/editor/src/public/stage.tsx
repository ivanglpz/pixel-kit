...
import {
  EditorPublicProjectAdapter,
  webEditorPublicProjectAdapter,
} from "../platform/public-projects";
...
const copyShareUrl = (text: string): void => {
...
export const PixelKitStagePublic = ({
  ...
  projectId,
  shareUrl,
  publicProjectAdapter = webEditorPublicProjectAdapter,
}: PixelKitStagePublicProps) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const SET_PUBLIC_PROJECT = useSetAtom(BUILD_PROJET_PUBLIC);
  const [browserShareUrl, setBrowserShareUrl] = useState("");
  const ID = projectId;

  const project = useQuery({
    enabled: Boolean(ID),
    queryKey: ["public_project", ID],
    queryFn: async () =>
      publicProjectAdapter.fetchPublicProject({ projectId: ID }),
  });
  ...
  useEffect(() => {
    ...
    setBrowserShareUrl(`${window.location.host}/project/${ID}`);
  }, [ID, shareUrl]);
  ...
  return (
    ...
    >
      <section className="absolute rounded-lg bg-white dark:bg-neutral-900 w-[260px] h-auto top-4 left-4 z-10 p-4 flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex flex-row gap-2">
            <input
              readOnly
              value={shareUrl ?? browserShareUrl}
            ...
            <button
              ...
              onClick={() => {
                copyShareUrl(shareUrl ?? browserShareUrl);
              }}
            >
              <Clipboard size={14} />
            </button>
